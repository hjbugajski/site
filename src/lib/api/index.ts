...
export const fetchGlobal = async <T = any>(slug: string): Promise<T | undefined> => {
  try {
    const req = await fetch(`${PAYLOAD_API}/globals/${slug}`, {
      method: 'GET',
      next: NEXT_CONFIG,
      headers: {
...
export const fetchPages = async (): Promise<PayloadPage[] | undefined> => {
  try {
    const req = await fetch(`${PAYLOAD_API}/pages`, {
      method: 'GET',
      next: NEXT_CONFIG,
      headers: {
        'Content-Type': 'application/json',
